...
def main(worlds, save = False):
    #runs the program for every World passed
    for key, world in worlds.items():
        ...
        alpha, beta, gammaQL, gammaSARSA, epsilon = 0.8 , 3, 0.9, 1, 0.1
        actions = np.array(['up', 'left', 'down', 'right']) #possible actions in each state
        methods = ['e-greedy']#['softmax', 'greedy','e-greedy']
        ...
        for learningMethod in ['SARSA', 'Q-Learning']:
            ...
            if learningMethod == 'SARSA':
                #FOR NON-SARSA FUNCTION COMMENTS, SEE "Q-Learning" ABOVE
                gamma = gammaSARSA
        ...
        for learningMethod in ['SARSA', 'Q-Learning']:
            for method in methods:
                ...
                ax.plot(xdata, ydata, label = f"{method} {learningMethod}")
                ...
                ax2.plot(range(len(averages)), averages, label = f"{method} {learningMethod}")
        ax.set_xlabel('Run')
...
if __name__ == "__main__":
    ...
    #creates a world with certain parameters
    worlds['exp1'] = World(20, 20, pitfalls, '0a', '19b', 500)
    ...
    for expNumber in range(3, 10):
        ...
        for pitfall in range(amountOfPitfalls):
            pitfalls.append(f"{np.random.randint(1,15)}{chr(np.random.randint(1,15) + 97)}")
    ...
    '''the random worlds below are smaller, but the start and beginning
        are always in the same place and the relative amount of pitfalls
        per world is higher than in the random worlds above'''
    ...
    ease the load on your PC and study each setup individually'''
    main(worlds) 